...
class Spyr(pyramid):
    ...
    def __init__(self, *args):    # (image height, filter file, edges)
        ...
        for i in range(ht):
            ...
            # works for sp1Filters, but not sp3Filters
            #print 'starting bfilts loop, len = %d' % (bfilts.shape[1])
            # FIX: there must be a better way of computing the starting filter
            #if bfilts.shape[1] <= 2:
            #    bidx = bfilts.shape[1]-1
            #else:
            #    bidx = (bfilts.shape[1] / 2) - 1
            #for b in range(bfilts.shape[1]-1,-1,-1):
            for b in range(bfilts.shape[1]):
    ...
    def reconPyrOrig(self, *args):
        ...
        for lev in range(Nlevs-1,-1,-1):
            if lev == Nlevs-1 and pyPyrUtils.LB2idx(lev,-1,Nlevs,Nbands) in reconList:
                ...
                pyPyrUtils.upConv(image = self.pyr[idx].copy(),
                                  filt = hi0filt.copy(), edges = edges,
            ...
            newSz = pyPyrUtils.nextSz(recon.shape, self.pyrSize)
            mult = (newSz[0] / recon.shape[0])
            # above worked with 2D but 1D needed this?!
            #mult = (newSz[0] / recon.shape[0]) - 1
            if newSz[0] % recon.shape[0] > 0:
                mult += 1
            if mult > 1:
                print 'flag 6'
    ...
    def __init__(self, *args):    # (image, height, filter1, filter2, edges)
        ...
        for ht in range(self.height, 1, -1):
            ...
            if len(im_sz) == 1 or im_sz[1] == 1:
                ...
                #                        stop = (los[ht].shape[1],
                #                                los[ht].shape[0]))
                # passes unit tests, but fails on circular edges
                #hi2 = pyPyrUtils.upConv(image = im, filt = filt2.T, 
                #                        edges = edges, step = (2,1), 
                #                        stop = (los[ht].shape[1],
                #                                los[ht].shape[0])).T
                # works with circular, but crashes unit tests
                #hi2 = pyPyrUtils.upConv(image = im, filt = filt2, 
                #                        edges = edges, step = (2,1), 
                #                        stop = (los[ht].shape[1],
                #                                los[ht].shape[0]))
                hi2 = pyPyrUtils.upConv(image = im, filt = filt2.T, 
        ...
        for lev in allLevs:
            ...
            if lev == 0:
                ...
                if res_sz[0] ==1 and lev in levs:
                    ...
                    #print 'filt'
                    #print hfilt
                    # doesn't work for circular edges
